...
export default function DashboardPage() {
  ...
  const stats = useMemo(() => {
    ...
    return { todayTasks, pendingTasks, completedTasks, todayBlocks, activeGoals, upcomingTasks }
  ...
  return (
    <div className="max-w-6xl mx-auto">
      <div className="mb-8">
        ...
        <p className="text-gray-500 mt-1">欢迎回来，开始规划你的一天吧</p>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        <StatCard icon={CheckSquare} label="待办任务" value={stats.pendingTasks.length} color="bg-blue-500" />
        <StatCard icon={Calendar} label="今日任务" value={stats.todayTasks.length} color="bg-purple-500" />
        <StatCard icon={Target} label="进行中目标" value={stats.activeGoals.length} color="bg-green-500" />
        <StatCard icon={Clock} label="今日时间块" value={stats.todayBlocks.length} color="bg-amber-500" />
      </div>
              ...
              {stats.upcomingTasks.map(task => (
                <div key={task.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                  <div className="flex items-center gap-3">
                    <div className={`w-2 h-2 rounded-full ${task.status === 'in_progress' ? 'bg-blue-500' : 'bg-gray-300'}`} />
