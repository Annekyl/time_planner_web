  ...
  return (
    <div className="max-w-6xl mx-auto">
      <div className="mb-6 md:mb-8 fade-in">
        ...
        <p className="text-gray-500 dark:text-gray-400 mt-1 text-sm md:text-base">欢迎回来，开始规划你的一天吧</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-6">
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-4 md:p-6 card-hover fade-in" style={{ animationDelay: '0.15s' }}>
          <h2 className="text-base md:text-lg font-semibold text-gray-800 dark:text-gray-100 mb-3 md:mb-4 flex items-center gap-2">
            <AlertCircle size={18} className="text-amber-500" />
            即将到来的任务
          </h2>
          {stats.upcomingTasks.length === 0 ? (
            <p className="text-gray-400 dark:text-gray-500 text-sm">暂无即将到来的任务</p>
          ) : (
            <div className="space-y-2 md:space-y-3">
              {stats.upcomingTasks.map(task => (
                <div key={task.id} className="flex items-center justify-between p-2.5 md:p-3 bg-gray-50 dark:bg-gray-700/50 rounded-lg transition-all duration-200 hover:bg-gray-100 dark:hover:bg-gray-700">
                  <div className="flex items-center gap-2 md:gap-3 min-w-0">
                    <div className={`w-2 h-2 rounded-full shrink-0 transition-colors ${task.status === 'in_progress' ? 'bg-blue-500 animate-pulse' : 'bg-gray-300 dark:bg-gray-600'}`} />
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-4 md:p-6 card-hover fade-in" style={{ animationDelay: '0.2s' }}>
          ...
          {stats.activeGoals.length === 0 ? (
            <p className="text-gray-400 dark:text-gray-500 text-sm">暂无进行中的目标</p>
          ) : (
            <div className="space-y-2 md:space-y-3">
              {stats.activeGoals.slice(0, 5).map(goal => (
                <div key={goal.id} className="p-2.5 md:p-3 bg-gray-50 dark:bg-gray-700/50 rounded-lg transition-all duration-200 hover:bg-gray-100 dark:hover:bg-gray-700">
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-sm font-medium text-gray-700 dark:text-gray-300 truncate mr-2">{goal.title}</span>
                    <span className="text-xs text-gray-500 dark:text-gray-400 shrink-0">{goal.progress}%</span>
                  </div>
                  <div className="w-full bg-gray-200 dark:bg-gray-600 rounded-full h-2 overflow-hidden">
                    <div className="bg-indigo-500 h-2 rounded-full progress-bar" style={{ width: `${goal.progress}%` }} />
                  </div>
                </div>
              ))}
            </div>
    ...
    <div className={`bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-3 md:p-5 card-hover ${className}`}>
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xl md:text-2xl font-bold text-gray-800 dark:text-gray-100 mt-0.5 md:mt-1">{value}</p>
        </div>
