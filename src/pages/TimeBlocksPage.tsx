...
export default function TimeBlocksPage() {
  ...
  const selectCls = "w-full px-3 py-2 border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-text-primary rounded-lg text-sm transition-all duration-200 focus:ring-2 focus:ring-brand focus:border-brand outline-none"
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-4 md:p-6 mb-4 md:mb-6 slide-down">
          ...
          <form onSubmit={handleSubmit} className="space-y-3 md:space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div><label className="block text-xs text-text-secondary mb-1">日期</label><input type="date" value={form.date} onChange={e => setForm(p => ({ ...p, date: e.target.value }))} className={selectCls} /></div>
              <div><label className="block text-xs text-text-secondary mb-1">颜色</label><input type="color" value={form.color} onChange={e => setForm(p => ({ ...p, color: e.target.value }))} className="w-full h-[38px] px-1 py-1 border border-gray-300 dark:border-gray-600 rounded-lg cursor-pointer" /></div>
              <div><label className="block text-xs text-text-secondary mb-1">开始时间</label><input type="time" value={form.start_time} onChange={e => setForm(p => ({ ...p, start_time: e.target.value }))} className={selectCls} /></div>
