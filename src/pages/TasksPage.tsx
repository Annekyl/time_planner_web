...
export default function TasksPage() {
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <div className="flex items-center justify-between mb-4 md:mb-6 fade-in">
        ...
        <div className="flex gap-2">
          <button
            ...
            className="px-3 md:px-4 py-2 text-xs md:text-sm border border-gray-300 rounded-lg hover:bg-gray-50 transition-all duration-200 btn-press"
          >
            分类
          </button>
          <button
      ...
      {showCategoryForm && (
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-3 md:p-4 mb-4 slide-down">
          ...
          <form onSubmit={handleAddCategory} className="flex items-center gap-2 md:gap-3">
            ...
            <input
              ...
              placeholder="分类名称"
              className="flex-1 min-w-0 px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500 outline-none transition-all duration-200"
              required
            />
          ...
          <div className="flex gap-2 mt-3 flex-wrap">
            {categories.map(c => (
              <span key={c.id} className="inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs fade-in" style={{ backgroundColor: c.color + '20', color: c.color }}>
      ...
      <div className="flex gap-2 md:gap-3 mb-3 md:mb-4 fade-in" style={{ animationDelay: '0.05s' }}>
        <select value={filterStatus} onChange={e => setFilterStatus(e.target.value)} className="px-2 md:px-3 py-2 border border-gray-300 rounded-lg text-xs md:text-sm bg-white min-w-0 flex-1 md:flex-none transition-all duration-200 focus:ring-2 focus:ring-indigo-500 outline-none">
          <option value="all">全部状态</option>
          <option value="pending">待办</option>
          <option value="in_progress">进行中</option>
          <option value="completed">已完成</option>
        </select>
        <select value={filterCategory} onChange={e => setFilterCategory(e.target.value)} className="px-2 md:px-3 py-2 border border-gray-300 rounded-lg text-xs md:text-sm bg-white min-w-0 flex-1 md:flex-none transition-all duration-200 focus:ring-2 focus:ring-indigo-500 outline-none">
      ...
      {showForm && (
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 md:p-6 mb-4 slide-down">
          ...
          <form onSubmit={handleSubmit} className="space-y-3 md:space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <select value={form.priority} onChange={e => setForm(p => ({ ...p, priority: Number(e.target.value) as 1 | 2 | 3 | 4 }))} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm transition-all duration-200 focus:ring-2 focus:ring-indigo-500 outline-none">
              ...
              <div>
                ...
                <select value={form.status} onChange={e => setForm(p => ({ ...p, status: e.target.value as any }))} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm transition-all duration-200 focus:ring-2 focus:ring-indigo-500 outline-none">
                  <option value="pending">待办</option>
                  <option value="in_progress">进行中</option>
                  <option value="completed">已完成</option>
                </select>
              ...
              <div>
                ...
                <input type="date" value={form.due_date} onChange={e => setForm(p => ({ ...p, due_date: e.target.value }))} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm transition-all duration-200 focus:ring-2 focus:ring-indigo-500 outline-none" />
              </div>
              <div>
                <label className="block text-xs text-gray-500 mb-1">分类</label>
                <select value={form.category_id} onChange={e => setForm(p => ({ ...p, category_id: e.target.value }))} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm transition-all duration-200 focus:ring-2 focus:ring-indigo-500 outline-none">
      ...
      <div className="space-y-2">
        {filteredTasks.map((task, i) => (
          <div
            key={task.id}
            className={`bg-white rounded-xl shadow-sm border border-gray-100 p-3 md:p-4 flex items-center gap-3 md:gap-4 transition-all duration-200 hover:shadow-md group fade-in ${task.status === 'completed' ? 'opacity-60' : ''}`}
            style={{ animationDelay: `${Math.min(0.03 * i, 0.3)}s` }}
          >
            <button
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-1.5 md:gap-2 flex-wrap">
                <span className={`text-sm font-medium transition-all duration-200 ${task.status === 'completed' ? 'line-through text-gray-400' : 'text-gray-800'}`}>
                  {task.title}
                </span>
                {task.category && (
            ...
            <div className="flex items-center gap-1 md:gap-2 shrink-0">
              ...
              <button onClick={() => handleEdit(task)} className="p-1.5 text-gray-400 hover:text-indigo-600 transition-all duration-200 opacity-0 group-hover:opacity-100 btn-press">
                <Edit3 size={14} />
              </button>
              <button onClick={() => deleteTask(task.id)} className="p-1.5 text-gray-400 hover:text-red-600 transition-all duration-200 opacity-0 group-hover:opacity-100 btn-press">
                <Trash2 size={14} />
              </button>
            </div>
          </div>
