  ...
  return (
    <div className="max-w-6xl mx-auto">
      <div className="flex items-center justify-between mb-4 md:mb-6 fade-in">
        <h1 className="text-xl md:text-2xl font-bold text-gray-800 dark:text-gray-100">日历</h1>
      ...
      {viewMode === 'month' && <MonthView days={monthDays} currentMonth={selectedDate} selectedDate={selectedDate} onSelectDate={setSelectedDate} getEventsForDay={getEventsForDay} />}
      {viewMode === 'week' && <WeekView days={weekDays} selectedDate={selectedDate} onSelectDate={setSelectedDate} getEventsForDay={getEventsForDay} />}
      {viewMode === 'day' && <DayView date={selectedDate} events={getEventsForDay(selectedDate)} />}
    </div>
  )
}

function MonthView({ days, currentMonth, selectedDate, onSelectDate, getEventsForDay }: { days: Date[]; currentMonth: Date; selectedDate: Date; onSelectDate: (d: Date) => void; getEventsForDay: (d: Date) => { tasks: any[]; blocks: any[] } }) {
  return (
    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-3 md:p-6 fade-in" style={{ animationDelay: '0.1s' }}>
      <div className="grid grid-cols-7 gap-0.5 md:gap-1">
        ...
        {days.map(day => { const inMonth = isSameMonth(day, currentMonth); const selected = isSameDay(day, selectedDate); const today = isToday(day); const events = getEventsForDay(day); return (
          <button key={day.toISOString()} onClick={() => onSelectDate(day)} className={`relative p-1 md:p-2 min-h-[48px] md:min-h-[80px] rounded-lg text-left transition-all duration-200 ${!inMonth ? 'text-gray-300 dark:text-gray-600' : selected ? 'bg-indigo-50 dark:bg-indigo-900/30 ring-2 ring-indigo-500 dark:ring-indigo-400' : 'hover:bg-gray-50 dark:hover:bg-gray-700/50'}`}>
            <span className={`text-xs md:text-sm ${today ? 'bg-indigo-600 text-white w-5 h-5 md:w-6 md:h-6 rounded-full flex items-center justify-center' : ''}`}>{format(day, 'd')}</span>
            <div className="mt-0.5 space-y-0.5 hidden md:block">{events.blocks.slice(0, 2).map(b => <div key={b.id} className="text-[10px] rounded px-1 truncate text-white" style={{ backgroundColor: b.color }}>{b.title}</div>)}{events.tasks.length > 0 && <div className="text-[10px] bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400 rounded px-1 truncate">{events.tasks.length} 个任务</div>}</div>
            <div className="mt-0.5 flex gap-0.5 md:hidden">{events.tasks.length > 0 && <div className="w-1.5 h-1.5 rounded-full bg-blue-400" />}{events.blocks.length > 0 && <div className="w-1.5 h-1.5 rounded-full bg-purple-400" />}</div>
          </button>
        )})}
      </div>
    </div>
...
function WeekView({ days, selectedDate, onSelectDate, getEventsForDay }: { days: Date[]; selectedDate: Date; onSelectDate: (d: Date) => void; getEventsForDay: (d: Date) => { tasks: any[]; blocks: any[] } }) {
    ...
    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden fade-in" style={{ animationDelay: '0.1s' }}>
      <div className="grid grid-cols-8 border-b border-gray-100 dark:border-gray-700">
        ...
        {days.map(day => { const selected = isSameDay(day, selectedDate); const today = isToday(day); return (
          <button key={day.toISOString()} onClick={() => onSelectDate(day)} className={`py-2 md:py-3 text-center border-l border-gray-100 dark:border-gray-700 transition-all duration-200 ${selected ? 'bg-indigo-50 dark:bg-indigo-900/30' : 'hover:bg-gray-50 dark:hover:bg-gray-700/50'}`}>
      ...
      <div className="overflow-y-auto max-h-[600px]">
        <div className="grid grid-cols-8 relative">
          ...
          {days.map(day => { const events = getEventsForDay(day); return (
            <div key={day.toISOString()} className="relative border-l border-gray-100 dark:border-gray-700">
              ...
              {events.tasks.length > 0 && <div className="absolute bottom-1 left-0.5 right-0.5">{events.tasks.slice(0, 1).map(t => <div key={t.id} className="bg-blue-500 text-white rounded px-1 py-0.5 text-[9px] truncate">{t.title}</div>)}{events.tasks.length > 1 && <div className="text-[9px] text-gray-400 dark:text-gray-500 px-1">+{events.tasks.length - 1}</div>}</div>}
            </div>
          )})}
        </div>
      </div>
...
function DayView({ date, events }: { date: Date; events: { tasks: any[]; blocks: any[] } }) {
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
      <div className="md:col-span-2 bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden fade-in" style={{ animationDelay: '0.1s' }}>
        <div className="overflow-y-auto max-h-[600px]"><div className="flex relative">
          ...
          <div className="flex-1 relative border-l border-gray-100 dark:border-gray-700">
      ...
      <div className="space-y-4">
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-4 md:p-6 fade-in" style={{ animationDelay: '0.15s' }}>
          <h3 className="font-semibold text-gray-800 dark:text-gray-200 mb-3 text-sm md:text-base">{format(date, 'M月d日 EEEE', { locale: zhCN })}</h3>
          <div className="mb-4">
            <h4 className="text-xs font-medium text-gray-500 dark:text-gray-400 mb-2 flex items-center gap-1.5"><span className="w-2 h-2 rounded-full bg-purple-500" />时间块 ({events.blocks.length})</h4>
            {events.blocks.length === 0 ? <p className="text-xs text-gray-400 dark:text-gray-500">无时间块</p> : <div className="space-y-1.5">{events.blocks.map(block => <div key={block.id} className="flex items-center gap-2 p-2 rounded-lg bg-gray-50 dark:bg-gray-700/50 transition-all duration-200 hover:bg-gray-100 dark:hover:bg-gray-700"><div className="w-1 h-8 rounded-full shrink-0" style={{ backgroundColor: block.color }} /><div className="min-w-0"><p className="text-sm text-gray-700 dark:text-gray-300 truncate">{block.title}</p><p className="text-[10px] md:text-xs text-gray-400 dark:text-gray-500">{block.start_time} - {block.end_time}</p></div></div>)}</div>}
          </div>
          <div>
            <h4 className="text-xs font-medium text-gray-500 dark:text-gray-400 mb-2 flex items-center gap-1.5"><span className="w-2 h-2 rounded-full bg-blue-500" />任务 ({events.tasks.length})</h4>
            {events.tasks.length === 0 ? <p className="text-xs text-gray-400 dark:text-gray-500">无任务</p> : <div className="space-y-1.5">{events.tasks.map(task => <div key={task.id} className="p-2 rounded-lg bg-gray-50 dark:bg-gray-700/50 transition-all duration-200 hover:bg-gray-100 dark:hover:bg-gray-700"><p className={`text-sm ${task.status === 'completed' ? 'line-through text-gray-400 dark:text-gray-500' : 'text-gray-700 dark:text-gray-300'}`}>{task.title}</p></div>)}</div>}
          </div>
        </div>
      </div>
    </div>
