...
export default function CalendarPage() {
  ...
  const handleAddSubmit = async (e: React.FormEvent) => {
    ...
    if (showAddModal === 'task') {
      ...
      await addTimeBlock({ title: formTitle, date: ds, start_time: formTime, end_time: `${endH}:${m}`, color: formColor, category_id: null, task_id: null })
    }
  ...
  const openModal = (type: 'task' | 'block', date?: Date) => {
      ...
      <AnimatePresence>
        {showAddModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="glass w-full max-w-sm rounded-2xl shadow-2xl border border-white/20 p-5 md:p-6 relative">
                ...
                {showAddModal === 'block' && (
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <input type="time" value={formTime} onChange={e => setFormTime(e.target.value)} className="w-full px-3 py-2 border border-white/20 bg-white/50 dark:bg-gray-800/50 text-gray-800 dark:text-gray-200 rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none text-sm" required />
                    </div>
                    <div>
                      <label className="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1.5">颜色标识</label>
...
function MonthView({ days, currentMonth, selectedDate, onSelectDate, getEventsForDay, onAddEvent }: { days: Date[]; currentMonth: Date; selectedDate: Date; onSelectDate: (d: Date) => void; getEventsForDay: (d: Date) => { tasks: any[]; blocks: any[] }; onAddEvent: (t: 'task'|'block', d: Date) => void }) {
  return (
    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-3 md:p-6 fade-in" style={{ animationDelay: '0.1s' }}>
      <div className="grid grid-cols-7 gap-0.5 md:gap-1">
        ...
        {days.map(day => { const inMonth = isSameMonth(day, currentMonth); const selected = isSameDay(day, selectedDate); const today = isToday(day); const events = getEventsForDay(day); return (
          <div key={day.toISOString()} onClick={() => onSelectDate(day)} className={`relative p-1 md:p-2 min-h-[64px] md:min-h-[96px] rounded-lg text-left transition-all duration-200 cursor-pointer group ${!inMonth ? 'text-gray-300 dark:text-gray-600' : selected ? 'bg-indigo-50 dark:bg-indigo-900/30 ring-2 ring-indigo-500 dark:ring-indigo-400' : 'hover:bg-gray-50 dark:hover:bg-gray-700/50'}`}>
...
function WeekView({ days, selectedDate, onSelectDate, getEventsForDay, onAddEvent }: { days: Date[]; selectedDate: Date; onSelectDate: (d: Date) => void; getEventsForDay: (d: Date) => { tasks: any[]; blocks: any[] }; onAddEvent: (t: 'task'|'block', d: Date) => void }) {
    ...
    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden fade-in" style={{ animationDelay: '0.1s' }}>
      <div className="grid grid-cols-8 border-b border-gray-100 dark:border-gray-700">
        ...
        {days.map(day => { const selected = isSameDay(day, selectedDate); const today = isToday(day); return (
          <div key={day.toISOString()} onClick={() => onSelectDate(day)} className={`group relative py-2 md:py-3 text-center border-l border-gray-100 dark:border-gray-700 transition-all duration-200 cursor-pointer ${selected ? 'bg-indigo-50 dark:bg-indigo-900/30' : 'hover:bg-gray-50 dark:hover:bg-gray-700/50'}`}>
      ...
      <div className="overflow-y-auto max-h-[600px]">
        <div className="grid grid-cols-8 relative">
          ...
          {days.map(day => { const events = getEventsForDay(day); return (
            <div key={day.toISOString()} className="relative border-l border-gray-100 dark:border-gray-700 cursor-crosshair" onDoubleClick={() => onAddEvent('block', day)}>
...
function DayView({ date, events, onAddEvent }: { date: Date; events: { tasks: any[]; blocks: any[] }; onAddEvent: (t: 'task'|'block', d: Date) => void }) {
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
      <div className="md:col-span-2 bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden fade-in" style={{ animationDelay: '0.1s' }}>
        <div className="overflow-y-auto max-h-[600px]"><div className="flex relative">
          ...
          <div className="flex-1 relative border-l border-gray-100 dark:border-gray-700 cursor-crosshair" onDoubleClick={() => onAddEvent('block', date)}>
