...
import { Play, Pause, RotateCcw, Timer, Minimize2, CloudRain, Coffee, Waves, VolumeX, GripHorizontal } from 'lucide-react'
        ...
        {!isOpen ? (
          ...
          >
            ...
            <div 
              ...
              onPointerDownCapture={e => e.stopPropagation()}
            >
              <button onClick={() => { setMode('work'); setIsActive(false); setTimeLeft(MODE_TIMES.work) }} className={`flex-1 py-1 md:py-1.5 text-xs font-bold rounded-lg transition-all ${mode === 'work' ? 'bg-white dark:bg-gray-700 text-indigo-600 dark:text-indigo-400 shadow-sm' : 'text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200'}`}>工作</button>
              <button onClick={() => { setMode('shortBreak'); setIsActive(false); setTimeLeft(MODE_TIMES.shortBreak) }} className={`flex-1 py-1 md:py-1.5 text-xs font-bold rounded-lg transition-all ${mode === 'shortBreak' ? 'bg-white dark:bg-gray-700 text-green-600 dark:text-green-400 shadow-sm' : 'text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200'}`}>短休</button>
              <button onClick={() => { setMode('longBreak'); setIsActive(false); setTimeLeft(MODE_TIMES.longBreak) }} className={`flex-1 py-1 md:py-1.5 text-xs font-bold rounded-lg transition-all ${mode === 'longBreak' ? 'bg-white dark:bg-gray-700 text-purple-600 dark:text-purple-400 shadow-sm' : 'text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200'}`}>长休</button>
            </div>
